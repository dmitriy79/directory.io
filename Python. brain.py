...
# Modified.  Added looping code and test to find brain wallets
import hashlib
import ctypes
import ctypes.util
import sys
...
# Set the target output address to determine the brain wallet for
target = "1F88jP4RM8cVhHTHxRGT4xpPmymqfy95tK"; # Brainwallet 'aaaaaabc' for testing.  

# The items to scan through to generate the brainwallet.  You can also use word lists like ['cat', 'dog', 'apple']
alphabet = "abcdefghijklmnopqrstuvwxyz0123456789"	
#alphabet = ['cat', 'dog', 'apple']
...
def test(otherversion):
	strings = [ alphabet, alphabet, alphabet ]
	for a in alphabet:
		for b in alphabet:
			for c in alphabet:
				for d in alphabet:
					for e in alphabet:
						for f in alphabet:
							for g in alphabet:
								for h in alphabet:
									all = a + b + c + d + e + f + g + h
									address = get_addr(gen_eckey(passphrase=all))
									if address[0] == target:
										print "Found for key:"+all
										print "Keys:",address
										sys.exit()
										raise SystemExit
